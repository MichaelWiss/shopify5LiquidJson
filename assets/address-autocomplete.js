/**
 * Address Autocomplete & ZIP → Province Mapper
 * Provides ZIP code to province/state mapping for US and Canada
 * Optionally integrates with Google Places API for full address autocomplete
 */

(function AddressAutocompleteModule() {
  'use strict';

  // ZIP → Province/State lookup tables
  const ZIP_MAPPINGS = {
    // US States by ZIP code prefix (first 3 digits)
    US: {
      ...
      '322': 'FL', '323': 'FL', '324': 'FL', '325': 'FL', '326': 'FL', '327': 'FL', '328': 'FL', '329': 'FL', '330': 'FL', '331': 'FL', '332': 'FL', '333': 'FL', '334': 'FL', '335': 'FL', '336': 'FL', '337': 'FL', '338': 'FL', '339': 'FL',
    ...
    // Canada postal code prefixes (first letter)
    CA: {
      ...
      'X': 'NU', // Nunavut/Northwest Territories
      'Y': 'YT'  // Yukon
    }
  };
  ...
  const AddressAutocomplete = {
    init() {
      ...
      const zipInputs = document.querySelectorAll('[data-address-zip]');
      const provinceSelects = document.querySelectorAll('[data-address-province]');
      
      if (zipInputs.length && provinceSelects.length) {
        this.bindZipMapping(zipInputs, provinceSelects);
      }
      
      // Optional: Initialize Google Places Autocomplete if API key available
      if (window.google && window.google.maps) {
        this.initGooglePlaces();
      }
    },

    bindZipMapping(zipInputs, provinceSelects) {
      zipInputs.forEach(zipInput => {
        zipInput.addEventListener('blur', () => {
          const zip = zipInput.value.trim().toUpperCase();
          const countrySelect = document.querySelector('[data-address-country]');
          const country = countrySelect ? countrySelect.value : 'US';
          
          const province = this.zipToProvince(zip, country);
          
          if (province) {
            provinceSelects.forEach(select => {
              const option = Array.from(select.options).find(
                opt => opt.value === province || opt.textContent.includes(province)
              );
              if (option) {
    ...
    zipToProvince(zip, country = 'US') {
      if (!zip) return null;
      
      if (country === 'US') {
        // Extract first 3 digits for US ZIP
        const prefix = zip.slice(0, 3);
        return ZIP_MAPPINGS.US[prefix] || null;
      } else if (country === 'CA' || country === 'Canada') {
        // Extract first letter for Canadian postal code
        const prefix = zip.charAt(0).toUpperCase();
        return ZIP_MAPPINGS.CA[prefix] || null;
      }
    ...
    fillAddressFields(addressComponents) {
      ...
      addressComponents.forEach(component => {
        ...
        if (addressMap[addressType]) {
          address[addressType] = component[addressMap[addressType]];
        }
      });
      ...
      if (streetInput && address.street_number && address.route) {
        streetInput.value = `${address.street_number} ${address.route}`;
      }
      
      if (cityInput && address.locality) {
        cityInput.value = address.locality;
      }
      
      if (provinceSelect && address.administrative_area_level_1) {
        provinceSelect.value = address.administrative_area_level_1;
      }
      
      if (zipInput && address.postal_code) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => AddressAutocomplete.init());
  } else {
